...
func_dir="/Users/junzhang/Documents/Topography/R_codes/Jun_test/functions" #where you store all the functions to be used

#if you are reading data directly from local computer
input_local="/Users/junzhang/Documents/Topography/CONUS2.0" #where you store all the input files, dem, mask, etc.
...
FolderName=paste("Str",riverorder,"Ep",ep,"2",sep="")#run name for PF files

# RunNameQGIS=paste("HSProc_Stream",riverorder,sep="")#runname for QGIS outs

outdir=paste("/Users/junzhang/Documents/Topography/R_codes/Jun_test/outputs/",FolderName,sep="") # store output files
```
...
setwd(func_dir)
source("./D4_Traverse.R")
source("./Init_Queue.R")
source("./Stream_Traverse.R")
source("./Find_Orphan.R")
source("./drainage_area.R")
# source("./Slope_Calc.R")
source("./Slope_Calc_Upwind.R")
```
...
# bordertypeR=raster("./Domain/1km_PF_BorderCellType.tif")#border cells with 1 for ocean borders and 2 for innerland borders
...
## calculate the slope and drainage area
...
SlopeName=paste(FolderName,"_unsmth.","mx",MaxSlope,".mn",-log10(MinSlope),".sec",SecTH,".up",sep="")#slope names 
...
slopesUW=SlopeCalcUP(dem=travHS$dem, mask=LakemaskT, direction=travHS$direction, dx=dx, dy=dy,  secondaryTH=SecTH, borders=Bord4Dir, maxslope=MaxSlope, upflag=T)
