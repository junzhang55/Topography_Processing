...
3. __Choices for slope calculation__. If choices for elevation and river network are kept the same, user can only run Section 5 for slope processes.
...
###If you've done it correctly this should work without any additional transforming on the matrix
```{r}
...
##D4 numbering. Numbers to assign to flow directions ordered, down, left, top, right.
```{r}
d4=c(1,2,3,4) 
```
...
SlopeName=paste(FolderName,"_unsmth.","mx",MaxSlope,".mn",-log10(MinSlope),".sec",SecTHName,UpName,sep="")
```
###Change to PF format
```{r}
