...
outdir="/Users/junzhang/Documents/Topography/R_codes/Jun_test/outputs/github_funcs/test4" # store output files
...
#make a mask with the sinks removed too by combinind with teh Lakemask
sinkindT=which(sinksT==2)
SinkmaskT=LakemaskT
SinkmaskT[sinkindT]=0
...
init=InitQueue(demT, initmask=channelmT, border=AllBord) #outputs include the init$mask (channel mask), init$queue(x,y and elevation of cells in the queue), init$marked(cells marked as processed), init$basins(basin number)
...
slopesUW=SlopeCalcUP(dem=travHS$dem, mask=LakemaskT, direction=travHS$direction, dx=dx, dy=dy,  secondaryTH=0.5, borders=AllBord, upflag=T)
