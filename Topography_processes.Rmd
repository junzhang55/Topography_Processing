...
rm(list=ls())
UserWD="/Users/junzhang/Topo_process" #Specify user directory
setwd(UserWD)
...
SmthFlag=cbind(T,F) #Whether the river networks are smoothed.
```
...
```{r}
ReqPackages=c('fields','raster','sp','rgdal')
...
for(maxi in 1:length(MaxSlope)){
  ...
  for (mini in 1:length(MinSlope)){
    ...
    for(seci in 1:length(SecTH)){
      ...
      SlopeName=paste(FolderName,"_unsmth.","mx",MaxStem,".mn",-log10(MinStem),".sec",SecTHName,UpName,sep="")
...
#6. Upload files to Avra if the upload flag is true
```{r}
if (UploadFlag==T){
